...
{
    public class TripleDiagonalLeft : Shape
    {
        private Block[,] _blockMatrix;

        public TripleDiagonalLeft()
        {
        ...
        public TripleDiagonalLeft(Point location) : base(location)
        {
            InitializeBlockMatrix();
            Draw();
            SetLeadBlock();
        ...
        {
            var shape = new TripleDiagonalLeft(this.Location);
            return shape;
        }
